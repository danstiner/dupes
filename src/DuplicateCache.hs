...
data DuplicateCacheF m next = List (Producer HashPath m () -> next)
...
remove cache path hash = do
  ...
  where
    remove' (DuplicateCache db _ writeOptions) = DB.delete db writeOptions
                                                   (toHashPathKey $ HashPath hash path)

addPath :: MonadResource m => DuplicateCache -> FilePath -> FileHash -> m ()
addPath (DuplicateCache db _ writeOptions) path hash = DB.put db writeOptions (toPathKey path)
                                                         (toPathValue hash)
...
list (DuplicateCache db readOptions _) = producerFromIterator db readOptions go
  where
    ...
    convert = forever $ await >>= \(key, value) ->
                          yield (HashPath (fromPathValue value) (fromPathKey key))
