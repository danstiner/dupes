...
-- ----------------------------------------------------------------------------- Interleave streams
-- non-deterministically with look-ahead
-- -----------------------------------------------------------------------------
-- | The input descriptor for maybe 'Merge' or 'MergeT'

data MY a b c where
        MaybeX :: MY a b (Maybe a)
        MaybeY :: MY a b (Maybe b)
        JustX :: MY a b a
        JustY :: MY a b b
...
-- | A 'Machine' that can read from two input stream in a non-deterministic manner with look-ahead
-- and monadic side-effects.
type MaybeMT m a b c = MachineT m (MY a b) c

merge :: Monad m => ProcessT m a a' -> ProcessT m b b' -> MaybeMT m a' b' c -> MaybeMT m a b c
merge ma mb m = MachineT $ runMachineT m >>= \v ->
                             case v of
                               Yield o k -> return $ Yield o (merge ma mb k)
                               Stop -> return Stop
                               Await f MaybeX _ -> runMachineT ma >>= \u ->
                                                     case u of
                                                       Yield a k -> runMachineT . merge k mb $ f $ Just
                                                                                                     a
                                                       Stop -> runMachineT $ merge stopped mb $ f
                                                                                                  Nothing --ff

                                                       Await g Refl fg -> return .
                                                                          Await
                                                                            (\b ->
                                                                               case b of
                                                                                 Just a -> merge
                               ...
                               Await f MaybeY _ -> runMachineT mb >>= \u ->
                                                     case u of
                                                       Yield b k -> runMachineT . merge ma k $ f $ Just
                                                                                                     b
                                                       Stop -> runMachineT $ merge ma stopped $ f
                                                                                                  Nothing -- ff
                               ...
                               Await f JustX ff -> runMachineT ma >>= \u ->
                                                     case u of
                                                       ...
                                                       Await g Refl fg -> return . Await (\a -> merge (g a) mb $ encased v) JustX
                                                                                   . merge fg mb $ encased
                                                                                                     v
                               Await f JustY ff -> runMachineT mb >>= \u ->
                                                     case u of
