...
diff compare' = go
  where
    ...
    goLeft l (rval,r) = lift (next l) >>= either (\_ -> yield (RightOnly rval) >> consumeRight r) (\(lval,l') -> emit lval rval l' r)
    goRight r (lval,l) = lift (next r) >>= either (\_ -> yield (LeftOnly lval) >> consumeLeft l) (\(rval,r') -> emit lval rval l r')
    emit lval rval l r = case compare' lval rval of
